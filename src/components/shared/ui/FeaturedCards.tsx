    ...
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 pt-5">
      {products.map((item) => (
        <Link
          ...
          key={item.id}
          className="bg-white p-5 header__mobile_menu border"
        >
          <Image src={item.image} alt={item.title} width={500} height={500}  layout="responsive" priority={true} sizes="(max-width: 768px) 100vw, 500px"  />
          <div>
            <h3 className="py-4 text-black text-center font-medium mb-4">{item.title}</h3>
